...
export function ChatCard() {
  ...
  useEffect(() => {
    const fetchChat = async () => {
      ...
      if (isLoggedIn) {
        try {
          const chatResponse = await axiosInstance.get(`/api/v1/chat/${chatId}`);
  ...
  if (isLoading) {
    return (
      <Container className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="relative">
          ...
          <div className="relative backdrop-blur-xl bg-linear-to-br from-slate-900/90 via-purple-900/70 to-slate-900/90 border border-cyan-500/30 rounded-2xl p-8 shadow-[0_0_50px_rgba(0,255,255,0.15)]">
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 border-2 border-cyan-400/30 border-t-cyan-400 rounded-full animate-spin"></div>
              <span className="text-cyan-300 text-lg font-medium">Loading chat...</span>
            </div>
          </div>
        </div>
      </Container>
