                    ...
                    <div className="space-y-5">
                        <Input 
                            ...
                            {...register("email",{
                                required: true,
                                validate: {
                                    matchPattern: (value) => /^(?=.{1,254}$)(?=.{1,64}@)(?![.])(?!.*\.\.)[A-Za-z0-9.!#$%&'*+/=?^_`{|}~-]+(?<![.])@(?:(?!-)[A-Za-z0-9-]{1,63}(?<!-)\.)+[A-Za-z]{2,63}$/.test(value) || "Invalid email address",
                                }
                            }
                            )}
                        />
                        <Input 
                            ...
                            {...register("password",{
                                required: true,
                                validate: {
                                    matchPattern: (value) => /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/.test(value) || "Password must contain at least 8 characters, including at least one letter and one number",
                                }
                            })}
                        />
                    </div>
